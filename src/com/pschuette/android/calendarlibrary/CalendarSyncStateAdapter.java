...
public class CalendarSyncStateAdapter extends BaseAdapter implements OnCheckedChangeListener{
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(convertView == null){
			...
			convertView = inflater.inflate(R.layout.calendarsyncstate_view, parent, false);
		}
		CalendarData calData = getItem(position);
		
		ImageView calColorImage = (ImageView) convertView.findViewById(R.id.calendarsyncstate_color);
		calColorImage.setBackgroundColor(calData.getColor());
		
		Switch calSyncSwitch = (Switch) convertView.findViewById(R.id.calendarsyncstate_switch);
	...
	public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
		...
		if(syncStateChangeListener != null){
			syncStateChangeListener.onSyncStateChanged(data, (Switch) buttonView);
		}
	}
	
	public void registerSyncStateChangeListener(OnSyncStateChangedListener syncStateChangeListener){
		this.syncStateChangeListener = syncStateChangeListener;
	}
	
	public void unregisterSyncStateChangeListener(OnSyncStateChangedListener syncStateChangeListener){
